...
public class MicHandle : MonoBehaviour {
 ...
 //   }
  ...
  private void DeriveBlow() {
    ...
    //  resultDisplay.GetComponent<Text>().text = "RMS: " + rmsValue.ToString("F2") + " (" + dbValue.ToString("F1") + " dB)\n" + "Pitch: " + pitchValue.ToString("F0") + " Hz";
        if(Mathf.Abs(dbValue/6)<3.5f){
		if(fireLight.GetComponent<Light>().intensity<14)
		fireLight.GetComponent<Light>().intensity += lightStep * Time.deltaTime * Mathf.Abs(dbValue/6);
